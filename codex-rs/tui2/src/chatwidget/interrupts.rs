...
impl InterruptManager {
    ...
    pub(crate) fn take_request_user_input(&mut self) -> Vec<RequestUserInputEvent> {
        let mut request_events = Vec::new();
        let mut retained = VecDeque::new();

        while let Some(interrupt) = self.queue.pop_front() {
            match interrupt {
                QueuedInterrupt::RequestUserInput(ev) => request_events.push(ev),
